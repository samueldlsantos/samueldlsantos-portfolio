...
const WorkExperience = ({experiences} : WorkExperienceProps) => {
  return (
    <div className="max-w-6xl mx-3 md:mx-4 lg:mx-5 xl:mx-auto mt-10">
        <h2 className="text-white text-center text-4xl font-bold mb-10">Experiencia laboral</h2>
        {experiences.length > 0 ? experiences.map((experience) => 
        <WorkItem key={experience.id} experience={experience}/>
        ):
        <p className="text-white font-bold text-center">No hay datos para mostrar</p>
      }
    </div>
  )
}
